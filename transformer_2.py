...
parser.add_argument('--epochs', type=int, required=False, help='How many epochs to train for', default=7)
...
train_data,test_data = train_test_split(train,test_size=.25)
# associate the text in the 'English' column with the EN_TEXT field, # and 'French' with FR_TEXT

train_data.to_csv("train_data.csv", index=False)
valid_data.to_csv("val_data.csv", index=False)
test_data.to_csv("test_data.csv", index=False)

data_fields = [('English', english), ('French', german)]
train_data,valid_data,test_data = TabularDataset.splits(path='./', train='train_data.csv', validation='val_data.csv',test="test_data.csv", format='csv', fields=data_fields)
'''
...
class Transformer(nn.Module):
    ...
    def forward(self, src, trg):
        ...
        #print("----")
        #print(src_seq_length,N)
        #print(trg_seq_length,N)
        #print("----")
...
#sentence = "un cheval passe sous un pont à côté d'un bateau."
