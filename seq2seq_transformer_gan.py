...
parser.add_argument('--file_name', type=str, help="Name of checkpoint file", default="transformer_gan.pth.tar")
parser.add_argument('--LAMBDA', type=float, help="Scaler for the adversarial term of the loss function", default=0.1)
...
classifier_optimizer = torch.optim.Adam(sentence_classifier.parameters(), lr=1e-5)
...
for epoch in range(num_epochs):
    ...
    for batch_idx, batch in enumerate(valid_iterator):
        ...
        target_flattend = target[1:].reshape(-1)

        loss = criterion(output_flattened, target_flattend)
        ...
        classifier_output = sentence_classifier(output.argmax(dim=2))
        classifier_test_loss = binary_cross_entropy_with_logits(
            classifier_output, torch.zeros_like(classifier_output)
        )
        classifier_test_running_loss.append(classifier_test_loss.item())


    print(f"Translated example sentence: \n {translated_sentence}")
    mean_loss = sum(valid_lossed) / len(valid_lossed)
    mean_loss_d = sum(classifier_test_running_loss) / len(classifier_test_running_loss)
    print(f"Mean loss for epoch {epoch + 1}: {mean_loss}")
    print(f"Discriminator mean loss for epoch {epoch + 1}: {mean_loss_d}")
    model.train()
    ...
    for batch_idx, batch in enumerate(train_iterator):
        ...
        sentences = torch.cat((target[1:, :], output.argmax(dim=2)), dim=1)
        # print(target.shape)
        # print(output.shape, "output")
        labels = torch.tensor([REAL_SENTANCE_LABEL]*target.shape[1] + [0.]*output.shape[1]).to(device)

        # Forward pass
        classifier_optimizer.zero_grad()
        # print(sentences.shape, "sents")
        # print(sentences)
        sentence_preds = sentence_classifier(sentences)
        # print(sentence_preds.shape)
        # print(labels.shape)
        classifier_loss = classifier_criterion(sentence_preds.flatten(), labels)
